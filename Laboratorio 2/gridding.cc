...
/*
La clase Matrix es la encargada de almacenar los datos procesados por las Tareas. Esta de es de acceso 
compartido y es de tipo Mutex para evitar problemas de concurrencia. La clase cuenta con 3 matrices de
tipo double, una para la parte real, otra para la parte imaginaria y otra para el peso. Ademas, cuenta
con un atributo N que indica el tamaño de la matriz.
*/
_Mutex  class Matrix{
private:
    int N;
...
public:
    ...
    /*
    Entrada: Un entero N que indica el tamaño de la matriz
    Salida: Ninguna
    Descripcion: Constructor de la clase, recibe el tamaño de la matriz, construye las matrices y las inicializa en 0.0
    */
    Matrix(int N): N(N){
...
public:
    ...
    que indica el tamaño de la celda, un entero N que indica el tamaño de la matriz y un puntero a la clase Matrix
    Salida: Ninguna
    Descripcion: Constructor de la clase, recibe el id de la tarea, un puntero a la corrutina Reader, el tamaño de la celda,
    el tamaño de la matriz y un puntero a la clase Matrix
    */
    Gridder(int id, Reader* r, double delta_x, int N, Matrix* m): id(id), r(r), delta_x(delta_x), N(N), m(m){};
...
private:
    ...
    Matrix* m; // Puntero a la clase Matrix
    vector <string> lineas; // Vector que contiene las lineas leidas
    ...
    void main(){
        ...
        while(true){
            ...
            for(int i = 0; i < lineas.size(); i++){
                ...
                // Se calcula el valor de las matrices en la posicion i_k, j_k de acuerdo al enunciado
                calculo_real = m->getReal(i_k, j_k) + (vr * w);
                calculo_imag = m->getImag(i_k, j_k) + (vi * w);
                calculo_peso = m->getPeso(i_k, j_k) + w;

                // Se actualiza el valor de las matrices en la posicion i_k, j_k
...
int main(int argc, char *argv[]) {
    ...
    Matrix* matrix = new Matrix(N); // Se crea la clase matriz
    ...
    for (int i = 0; i < num_tasks; i++) { // Se crean las tareas Gridder
        Gridders[i] = new Gridder(i, ReaderPtr, delta_x, N, matrix);
    ...
    cout << "Tiempo del proceso de gridding con matriz compartida: " << time2 << "[s]" << endl; // Se imprime el tiempo de ejecucion

    output_directory_r = output_directory + "r.raw"; // Se crea el nombre del archivo de salida de la parte real
    ...
    for (int i = 0; i < N; i++) { // Escribir los datos de la matriz real en el archivo .raw
        archivo_r.write(reinterpret_cast<char*>(matrix->matrix_real[i]), N * sizeof(double));
    }
    archivo_r.close();   // Se cierra el archivo de salida
    cout << "Archivo de salida de la parte real creado" << endl;

    output_directory_i = output_directory + "i.raw"; // Se crea el nombre del archivo de salida de la parte imaginaria
    ...
    for (int i = 0; i < N; i++) { // Escribir los datos de la matriz imaginaria en el archivo .raw
        archivo_i.write(reinterpret_cast<char*>(matrix->matrix_imag[i]), N * sizeof(double));
    }
